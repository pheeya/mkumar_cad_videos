...
req.onload = function (e) {
  ...
  for (let i = 0; i < json_sheet.length; i++) {
    ...
    // cad video title 
    var title_large = document.createElement("div");
    title_large.classList.add("time");
    card.appendChild(title_large)
    //footer div
    ...
    company.src = s["Source Image"]
    price.src = s['Price Image'];
    illustration.src = s["Link to Image"];
    by.innerHTML = `Submitted By: ${s['Your Name']}`
    ...
    card.dataset.modelTitle= s["Model Title"];
    card.dataset.softwareUsed = s["Software Used"];
    card.dataset.categories = s["Categories"];
    card.dataset.source = s["Source"];
    card.dataset.link = s.Link;
    card.dataset.time = time.innerHTML;
    card.dataset.by =  `Submitted By: ${s['Your Name']}`
    card.dataset.postedBy = s['Timestamp']
...
function create_filters(sheet) {
  ...
  options.forEach(function (fetched_option) { //option = filter label
    ...
    if (fetched_option === "Software Used" || fetched_option === "Categories" || fetched_option === "Source") {
...
function pop(element) {
  ...
  document.getElementById("view360Iframe").src= element.dataset['thumbnail'];
  document.getElementById("popup_time").innerHTML = element.dataset.by
